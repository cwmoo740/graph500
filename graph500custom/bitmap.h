...
static inline void bm_copy(bitmap_t* dest, bitmap_t* src)
{
    memcpy(dest->start, src->start, sizeof(uint64_t)*((src->end)-(src->start)));
}

static inline void bm_or(bitmap_t* output, bitmap_t* mask)
{
    #pragma omp parallel for
    for(uint64_t* it=mask->start; it<mask->end; it++)
    {
        if (bm_get_bit(mask, it-mask->start))
        {
            bm_set_bit(output, it-mask->start);
        }
    }
}
